...
module Bare : sig
  ...
  (** Exception raised whenever a valid endpoint is garbage collected,
meaning that it is unused. *)
...
valid endpoints and dual types. *)
 ...
 invalid.  *)
  ...
  (** {2 Low-level message-passing primitives} *)
...
@raise InvalidEndpoint if the endpoint [ep] is invalid. *)
  ...
  (** [accept ep] receives a message from the endpoint [ep] with input
 capability.  @return the received message.  @raise InvalidEndpoint if
 the endpoint [ep] is invalid.  *)
  val accept : 'm it -> 'm
...
endpoint [ep]. *)
  ...
  (** {2 Combinators for sequential composition} *)
